...
#if defined(NDEBUG)
#error "xnode cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uxnode) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateXNODESupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating XNODE supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating XNODE supply..."), percent);
		}
...
int64_t GetBlockValue(int nHeight)
{
    int64_t nSubsidy = 0;
    if (nHeight == 1) {
        nSubsidy = 1050000 * COIN; // PREMINE 0.5% OF TOTAL SUPPLY 210 000 000
    } else if (nHeight > 1 && nHeight <= 1440) { 
        nSubsidy = 100 * COIN;
    } else if (nHeight > 1440 && nHeight <= 25000) {
        nSubsidy = 0.40 * COIN;
        ...
        nSubsidy = 25 * COIN;
    } else if (nHeight > 200000 && nHeight <= 225000) {
        nSubsidy = 30 * COIN;
    } else if (nHeight > 225000 && nHeight <= 250000) {
        nSubsidy = 35 * COIN;
    } else if (nHeight > 250000 && nHeight <= 275000) {
        nSubsidy = 40 * COIN;
    } else if (nHeight > 275000 && nHeight <= 300000) {
        nSubsidy = 45 * COIN;
    } else if (nHeight > 300000 && nHeight <= 325000) {
        nSubsidy = 50 * COIN;
    } else if (nHeight > 325000 && nHeight <= 350000) {
        nSubsidy = 55 * COIN;
    } else if (nHeight > 350000 && nHeight <= 375000) {
        nSubsidy = 60 * COIN;
    } else if (nHeight > 375000 && nHeight <= 400000) {
        nSubsidy = 65 * COIN;
    } else if (nHeight > 400000 && nHeight <= 425000) {
        nSubsidy = 70 * COIN;
    } else if (nHeight > 425000 && nHeight <= 450000) {
        nSubsidy = 75 * COIN;
    } else if (nHeight > 450000 && nHeight <= 475000) {
        nSubsidy = 80 * COIN;
    } else if (nHeight > 475000 && nHeight <= 500000) {
        nSubsidy = 90 * COIN;
...
{
    util::ThreadRename("xnode-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> xnodeInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                xnodeInputs.push_back(stakeIn);
        }
        const bool hasXNODEInputs = !xnodeInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasXNODEInputs) {
                    ...
                    for (const CTxIn& xnodeIn : xnodeInputs)
                        if(xnodeIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasXNODEInputs) {
                            for (const CTxIn& stakeIn : xnodeInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // xnode: We use certain sporks during IBD, so check to see if they are
