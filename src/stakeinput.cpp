...
bool CXNODEStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CXNODEStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CXNODEStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CXNODEStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CXNODEStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CXNODEStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CXNODEStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CXNODEStake::GetUniqueness() const
{
    //The unique identifier for a XNODE stake is the outpoint
...
CBlockIndex* CXNODEStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CXNODEStake::ContextCheck(int nHeight, uint32_t nTime)
{
