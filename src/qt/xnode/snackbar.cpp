...
#include "qt/xnode/forms/ui_snackbar.h"
#include "qt/xnode/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(xnodeGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &xnodeGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
