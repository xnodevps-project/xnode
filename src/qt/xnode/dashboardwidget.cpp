...
#include "qt/xnode/dashboardwidget.h"
#include "qt/xnode/forms/ui_dashboardwidget.h"
#include "qt/xnode/sendconfirmdialog.h"
#include "qt/xnode/txrow.h"
...
DashboardWidget::DashboardWidget(xnodeGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareXNODE, "square-chart-xnode");
    setCssProperty(ui->labelXNODE, "text-chart-xnode");
    ...
    setCssProperty(ui->labelAmountXNODE, "text-stake-xnode-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &xnodeGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &xnodeGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &xnodeGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            xnode = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalXNODE += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesXNODE.append(xnode);

        int max = std::max(xnode, xnode);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesXNODE);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalXNODE > 0) {
        setCssProperty(ui->labelAmountXNODE, "text-stake-xnode");
    } else {
        setCssProperty(ui->labelAmountXNODE, "text-stake-xnode-disable");
    }
    forceUpdateStyle({ui->labelAmountXNODE});
    ui->labelAmountXNODE->setText(GUIUtil::formatBalance(chartData->totalXNODE, nDisplayUnit));
