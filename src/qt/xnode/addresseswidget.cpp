...
#include "qt/xnode/addresslabelrow.h"
#include "qt/xnode/addnewaddressdialog.h"
#include "qt/xnode/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(xnodeGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination xnodeAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(xnodeAdd, label.toUtf8().constData(),
