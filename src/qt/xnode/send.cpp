...
#include "qt/xnode/forms/ui_send.h"
#include "qt/xnode/addnewcontactdialog.h"
#include "qt/xnode/qtutils.h"
#include "qt/xnode/sendchangeaddressdialog.h"
#include "qt/xnode/optionbutton.h"
#include "qt/xnode/sendconfirmdialog.h"
#include "qt/xnode/myaddressrow.h"
#include "qt/xnode/guitransactionsutils.h"
...
SendWidget::SendWidget(xnodeGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-xnode");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination xnodeAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(xnodeAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(xnodeAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
