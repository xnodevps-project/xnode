...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter xnode Coin address (e.g. %1)").arg("M6tGR83SQbie1SW72hjcWJvFfip5krte2Z"));
...
{
    // return if URI is not valid or is no xnode: URI
...
{
    // Convert xnode:// to xnode:
    //
    //    Cannot handle this later, because xnode:// will cause Qt to see the part after // as host,
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "xnode.lnk";
}
...
{
    return GetAutostartDir() / "xnode.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a xnode.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=xnode Coin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the xnode app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add xnode app to startup item list
