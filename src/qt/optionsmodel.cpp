...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (settings.contains("nAnonymizexnodeAmount") || reset)
        SoftSetArg("-anonymizexnodeamount", settings.value("nAnonymizexnodeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in xnode.cpp during shotdown to also remove the window positions
