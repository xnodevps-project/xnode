...
*Setup instructions for a gitian build of xnode using a VM or physical system.*

Gitian is the deterministic build process that is used to build the xnode
...
to the xnode GitHub release page.
...
- [Building xnode](#building-xnode-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/xnode-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/xnode-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/xnode-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/xnode-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/xnode-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/xnode-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/xnodevps-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@xnode.trade) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/xnode-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/xnode-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/xnode-osx-*-build.assert
```
