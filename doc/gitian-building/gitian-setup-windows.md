...
The build script we'll be using is contained in the xnode github repository ([contrib/gitian-build.py](https://github.com/xnodevps-project/xnode/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the xnode repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/xnodevps-project/xnode/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, xnode-detached-sigs, and xnode GitHub repos.
